...
class UNetBasic(nn.Module):
    def __init__(self, input_channels=1, n_filters = 128, dropout = 0.1, batchnorm = False, isFirst=False):
    ...
    def pass_through_conv2d_block(self, x, conv_block, apply_skip=True, isFirst=False):
        ...
        x = conv_block[3](x)
        if isFirst:
            return x, first_output
        return x
        
    def forward(self, x):
        
        if self.isFirst:
            c1, first_output = self.pass_through_conv2d_block(x, self.c1, apply_skip=False, isFirst=True)
        else:
